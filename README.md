...
任务恢复补充：目标交易周在下一交易周首个开市日开始前始终属于当周正式窗口；周初、日报和周终错过计划执行时间后仍可幂等补生成正式结果。补生成不会移动信息边界：周初仍截至首个交易日前一交易日 15:00，日报截至目标交易日 15:00，周终截至最后交易日收盘。Worker 重启会补做已到期但未完成的任务，并每 30 分钟检查当周已结束交易日的缺失日报（收盘后也检查当天）；历史页周目录按正式名单、日报、复盘和成功完成的隔离整周回溯取并集。

手动每日简报：决策管理的“每日简报”先按日期补齐请求当周已到期的缺失日报，再处理当日；已有当日日报不再阻止前几日补缺。休市或当日未到 15:30 时只处理已到期日期，保留各日期原定截止点，不跨周补写正式数据。某日失败不撤销其他日期成功结果，任务保留逐日审计并报告失败，再次点击只补仍缺失的日期。此行为由 `POST /api/v1/jobs/output` 的 `catch_up_week: true` 启用，旧客户端默认仍为单日任务。

运行可靠性（2026-09-08）：调度线程共用单一持久异步事件循环，避免 asyncpg 连接跨循环；日报/复盘队列独立于耗时周初准备，输出任务在专用执行通道串行执行。调度允许迟到 6 小时并合并积压触发；周初全量快照覆盖失败时最多定向补抓两轮缺失标的，仍不完整则失败，不降低质量门禁。Sina 同步调用使用可终止子进程，超时后回收进程。以上修复不能保证免费数据源永不失败，也不自动审批或发布名单。故障证据、补跑状态和后续建议见 [2026-09-08 故障与代码审查](docs/reviews/2026-09-08-reliability-and-code-review.md)。
